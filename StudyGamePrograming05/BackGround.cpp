...
BackGround::BackGround(Game* game) :Actor(game)
{
	...
	bgactor->SetPosition(Vector2(game->mWindowWidth * (-0.5f), 0.0f));
...
void BackGround::UpdateActor(float deltaTime)
{
	...
	for (auto bg : mBGs)
	{
		if (bg->GetPosition().x < GetGame()->mWindowWidth * (-1.0f))
		{
			bg->SetPosition(bg->GetPosition() + Vector2(GetGame()->mWindowWidth, 0.0f));
		}
		else if (bg->GetPosition().x > GetGame()->mWindowWidth * 1.0f)
		{
			bg->SetPosition(bg->GetPosition() - Vector2(GetGame()->mWindowWidth, 0.0f));
		}
		if (bg->GetPosition().y < GetGame()->mWindowHeight * (-1.0f))
		{
			bg->SetPosition(bg->GetPosition() + Vector2(0.0f, GetGame()->mWindowHeight));
		}
		else if (bg->GetPosition().y > GetGame()->mWindowHeight * 1.0f)
		{
			bg->SetPosition(bg->GetPosition() - Vector2(0.0f, GetGame()->mWindowHeight));
		}
	}
